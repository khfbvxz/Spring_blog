...
public class BookController {
	...
	@GetMapping("/book")
	public ResponseEntity<?> findAll(){ // 물음표 어떤 것을 리턴할지 모를때 
		return new ResponseEntity<>(bookService.모두가져오기(),HttpStatus.OK);
	}
	
	@GetMapping("/book/{id}")
	public ResponseEntity<?> findById(@PathVariable Long id){ // 물음표 어떤 것을 리턴할지 모를때 
		return new ResponseEntity<>(bookService.한건가져오기(id),HttpStatus.OK);
	}
	@PutMapping("/book/{id}")
	public ResponseEntity<?> update(@PathVariable Long id, @RequestBody Book book){ // 물음표 어떤 것을 리턴할지 모를때 
